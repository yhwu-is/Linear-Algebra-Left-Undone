...
我们考虑如下的线性泛函 $ \varphi : V \rightarrow F, \enspace \varphi (v) = \langle Tv, w \rangle $，那么利用一下刚学到的\autoref{thm:23:Riesz}，存在唯一的 $ u \in W $，使得 $ \varphi (v) = \langle v, u \rangle $，再结合一下伴随的定义，只需要定义 $ T^*w = u $ 即可.
...
\begin{theorem} \label{thm:24:复内积空间}
    设 $ V $ 是复内积空间，$ T \in \mathcal{L}(V) $. 若 $ \forall v \in V,\enspace
...
\begin{theorem}
    若 $ T $ 是 $ V $ 上的自伴算子，$ \forall v \in V,\enspace
        \langle Tv, v \rangle = 0 $，则 $ T = 0 $.
\end{theorem}
...
\begin{example}
    设 $ V $ 是实内积空间， $ T \in \mathcal{L}(V),\enspace
        \forall v \in V,\enspace \langle Tv, v \rangle = 0 $. 证明：$ T^* = -T $.
\end{example}
...
\begin{theorem}
    算子 $ T \in \mathcal{L}(V) $ 是正规的当且仅当 $ \forall v \in V,\enspace
