...
\begin{theorem}
    \keyterm{极分解定理} 设 $ T \in \mathcal{L}(V) $，则存在一个等距同构 $ S \in \mathcal{L}(V) $，
    使得 $ T = S\sqrt{T^*T} $. 
\end{theorem}
...
\begin{definition}
    \keyterm{奇异值} 设 $ T \in \mathcal{L}(V) $，则 $ T $ 的奇异值就是 $ \sqrt{T^*T} $
...
\begin{theorem}
    \keyterm{奇异值分解（Singular value decomposition）}
