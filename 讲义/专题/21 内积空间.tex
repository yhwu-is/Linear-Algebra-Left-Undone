...
\begin{example} \label{ex:19:内积和范数}
    \begin{enumerate}
...
\begin{example} \label{ex:19:标准正交组}
...
\begin{example}
    \begin{enumerate}
        ...
        是 $\mathbf{R}[x]_2$ 上的线性泛函, 此处的内积为\autoref{ex:19:内积和范数} (2) 中定义的.
...
\begin{definition} \keyterm{正交补}[orthogonal complement]
    设 $ U $ 是 $ V $ 的子集，则 $ U $ 的正交补（记作 $ U^{\perp } $）
...
\begin{definition} \keyterm{正交投影}[orthogonal projection]
    设 $ U $ 是 $ V $ 的有限维子空间. 定义 $ V $ 到 $ U $ 上的正交投影为如下算子
