...
\begin{solution}
    \begin{enumerate}
        ...
        \begin{gather*}
            k_1 + 2k_2 - k_3 = 0 \\
            \frac{1}{2}k_1 + 2k_2 + \frac{1}{2}k_3 = 0 \\
            \frac{1}{3}k_1 + \frac{8}{3}k_2 - \frac{1}{3}k_3 = 0
        \end{gather*}
...
\begin{proof}
    我们又一次遇到了映射套映射的情况，那当然类似于之前对偶映射的讨论，我们的处理手段就是对需要定义的映射进行``取值计算''. 于是我们取$\psi$在$v\in V$上的值，得到$\psi(v)$，它是一个将线性泛函映到实数的线性泛函. 这仍然是映射套映射的情况，于是我们不妨更进一步，取$\psi(v)$在$f\colon V^*\to\R$上的值，我们有$(\psi(v))(f)$是一个实数，这时我们要素齐全，直接令
...
\begin{proof}
    \begin{enumerate}
        ...
        \end{align*}
        因此，$\psi$是线性映射.
        \item 这里可能有点抽象，我们还是用``取值运算''搞清楚$\sigma^{**}$与$\psi$究竟在干什么：
        \begin{enumerate}
