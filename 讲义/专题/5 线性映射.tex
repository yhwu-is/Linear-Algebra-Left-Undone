...
实际上，前面我们证明的线性映射加法和数乘的结果仍然是线性映射，就是线性映射的加法和数乘的封闭性，因此我们只需证明线性映射的加法和数乘满足线性空间的 8 条性质即可. 事实上这是非常显然的，我们只需注意 $\mathcal{L}(V_1,V_2)$ 中的零元就是零映射 $\theta$（将 $V_1$ 中的任意元素都映射到 $V_2$ 中的零元）即可，因此我们不再赘述. 关于该线性空间的维数我们放在\autoref{thm:线性映射与矩阵的同构} 中讨论.
...
我们需要注意的是，上述 \ref*{item:6:双射等价条件:1} 与 \ref*{item:6:双射等价条件:2} 等价不是基于线性映射基本定理得到的，而是在前述\autoref{thm:单射与核空间} 中已经证明的. 其余等价性的证明也是非常简单，只需要简单套用维数公式即可，证明留给读者完成.
    ...
    \item 关于核空间，我们有如下定理，这一定理在之后讨论矩阵标准形的时候非常有用：
          ...
          \begin{proof}
              \begin{enumerate}
                  ...
                  \item 由上一点知我们只需证$\ker \sigma^n=\ker \sigma^{n+1}$. 用反证法，则链条$\{0\}=\ker\sigma^0\subsetneqq\ker\sigma^1\subsetneqq\cdots\subsetneqq\ker\sigma^{n+1}$一定是递增的真包含链条（否则其中会有相邻两项相等，与反证假设矛盾），从而这一递增链条每处严格包含于的维数必然增加1，因此$\dim\ker\sigma^{n+1}\geqslant n+1>n$，但我们知道$\ker\sigma^{n+1}$是$V$的子空间，因此矛盾！故命题成立.
              \end{enumerate}
          \end{proof}
