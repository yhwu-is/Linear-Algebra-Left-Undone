...
\begin{enumerate}
    \item 线性空间还有一个重要的概念是运算封闭，即线性空间中的元素进行加法或数乘运算后，得到的元素仍然是属于线性空间的. 这一点是定义要求的，加法封闭是 Abel 群的要求，因为 Abel 群要求加法运算定义为映射 $V\times V\to V$，因此$V$中两个元素相加后必须仍在$V$中（事实上这是代数系统的共性），数乘注意前述定义中数乘运算``取值于$V$''的要求；
