...
在这一章中，我们将以范畴论的手法串联本书的两个高潮，其一，Galois 理论，出现在未竟专题当中，其二，Jordan 分解，则出现在最后几讲的正文当中. 因此，本章主要分四个部分进行，其中的每个部分都是浅尝辄止，为了保持叙述之灵氛，难以细致入微：

\begin{enumerate}
    \item 范畴论的基本概念和构造；
    \item Grothendieck-Galois 理论，或者说，关于下降（descent）的理论；
    \item 淡中-Krein 对偶，主要是关于张量范畴的理论，将会横跨正文之 Jordan 分解和未竟专题之 Witt 理论与群表示论；
    \item 总结，将这两种理论在更广泛的语境下统合，这在前面的呈现之后应当是明显的.
\end{enumerate}

其中第二、第三部分将会成为一系列抽象叙述的核心，而第一、第四部分则是对它们的铺垫和总结. 这两个部分的理论大约均成型于上世纪末到本世纪初，最终的总结与统合则虽然已有草创，但仍然不尽人意. 因此，将其作为未竟之美一书的结尾既是对全文的总结，亦是对未竟之功的展望. 自然，在这个急促的叙述过程当中，有许多枝节的意蕴亦不能被充分展现，但只要让读者意识到这种如 Grothendieck 所谓``仿佛来自虚空''的统一性，本章所耗费的大量笔墨与心力也就并非白费.

\section{范畴论的概念和构造}

\subsection{范畴、函子、自然变换}
...
单单是范畴、函子和自然变换显然什么都不能给出. 现在，我们无非是形式化了一些原来就已经说出的东西，顶多是最后对自然性的表述稍稍有些新意. 范畴论最核心的点实际上在于，它告诉了你很多被定义的东西实际上有其他的定义方式，这就是我们在下面几节会讨论的内容.
...
\section{Grothendieck 对 Galois 理论之重述}

\subsection{Stone 对偶：一个轻松的例子}

\subsection{函子范畴和单子}

\subsection{下降}

\subsection{范畴 Galois 理论}

\subsection{交换环上的 Galois 对应与代数几何之发端*}

\section{淡中范畴和淡中-Krein 对偶}

\subsection{Jordan 标准型的推广}

\subsection{张量范畴和表示范畴}

\subsection{淡中范畴及其对偶}

\section{统合与总结}

在这一节中，我们将在上面的基础上，用余单子的语言统合淡中-Krein 对偶和 Grothendieck-Galois 理论. 由于笔者学识以及本书篇幅所限，这一部分势必难以展现出其中丰富的细节，但读者若是有心，应当能够品味出其中的微妙联系. 而其中道之精微处，也是本书所欲付诸言筌而难竟的未竟之美.
