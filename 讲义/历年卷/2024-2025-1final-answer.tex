...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item
            求 $AX=0$ 可得 $X=(x_1,x_2,x_3,x_4)^\mathrm{T} = a(2,2,-1,0)^\mathrm{T} + b(0,1,-1,1)^\mathrm{T}$，核空间为$\spa\{(2,2,-1,0)^\mathrm{T}, (0,1,-1,1)^\mathrm{T}\}$.

            取出发空间的一组自然基$e_1,\ldots,e_4$，则$T(e_1)=(1,1,1)^\mathrm{T}$，$T(e_2)=(-1,0,1)^\mathrm{T}$，$T(e_3)=(0,2,4)^\mathrm{T}$，$T(e_4)=(1,2,3)^\mathrm{T}$. 从而可以对 $A$ 作初等行变换：
            \[
            \begin{pmatrix}
                1 & -1 & 0 & 1 \\
                1 & 0 & 2 & 2 \\
            ...
            \end{pmatrix}
            \]
            可得像空间的一组基为 $T(e_1), T(e_2)$.

            故像空间为$\spa\{(1,1,1)^\mathrm{T}, (-1,0,1)^\mathrm{T}\}$.
        \item 即求$P,Q$使得
            \[
            Q^{-1}AP =
            \begin{pmatrix}
