...
此前我们已经提到，根据奇异值分解的表达式，奇异值分解把一个一般的线性映射（矩阵）分解成了“旋转（或镜像）$+$ 拉伸 $+$ 旋转（或镜像）”三步. 本节希望更细致地描述这一点，进而揭示奇异值的几何意义.
...
总结上述证明过程，右奇异向量给出了 $T_A$ 出发空间中彼此正交的一系列方向，$T_A$ 在这些方向上的伸张倍数分别就是奇异值，最后左奇异向量给出这些方向在到达空间中旋转后得到的方向. 下面的示意图展示了二维情形下“单位圆 $\to$ 轴对齐椭圆 $\to$ 旋转后的椭圆”的过程.
...
\begin{summary}
    ...
    在应用上，奇异值分解既能帮助我们理解线性映射的几何本质，也能用来刻画算子范数，还能服务于数据降维、图像处理和数值计算等问题. 特征人脸算法便是其中一个典型例子.
\end{summary}
