...
\begin{proof}
    \begin{enumerate}
        ...
        \item 由线性映射基本定理（\autoref{thm:线性映射基本定理}）有 $\dim \im T = \dim V - \dim \ker T$. 同理，结合 \autoref{thm:伴随核空间像空间} 有 $\dim \im T^* = \dim V - \dim \ker T = \dim \im T$，再由第 3 条知 $\dim \im T^*T = \dim \im T^*$，因此 $\dim \im T = \dim \im T^* = \dim \im T^*T$.
    \end{enumerate}
\end{proof}
