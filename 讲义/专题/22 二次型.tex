...
\begin{proof}
    记 $E_{ij}$ 为第 $(i,j)$ 个位置为 $1$、其余位置为 $0$ 的矩阵.
    \begin{enumerate}
        \item 设 $P_{ij}$ 为交换第 $i,j$ 个基向量的置换矩阵，则 $P_{ij}^{-1}=P_{ij}^\mathrm{T}=P_{ij}$，而 $P_{ij}^\mathrm{T}AP_{ij}$ 恰好对应于同时交换 $A$ 的第 $i,j$ 行与第 $i,j$ 列.

        \item 设 $D_i(k)=I+(k-1)E_{ii}$，则 $D_i(k)$ 可逆且 $D_i(k)^\mathrm{T}=D_i(k)$. 直接计算可知 $D_i(k)^\mathrm{T}AD_i(k)$ 恰好对应于将 $A$ 的第 $i$ 行乘以 $k$，再将第 $i$ 列乘以 $k$.

        \item 设 $T_{ji}(k)=I+kE_{ji}\enspace(i\neq j)$，则 $T_{ji}(k)$ 可逆，且 $T_{ji}(k)^{-1}=I-kE_{ji}$. 由于 $T_{ji}(k)^\mathrm{T}=I+kE_{ij}$，所以 $T_{ji}(k)^\mathrm{T}AT_{ji}(k)$ 恰好对应于将第 $i$ 行乘以 $k$ 加到第 $j$ 行，再将第 $i$ 列乘以 $k$ 加到第 $j$ 列.
    \end{enumerate}
    因而上述三类变换都是合同变换.
\end{proof}
...
\begin{solution}
    ...
    \]
    因而该二次型的相合规范形中有 $5$ 个正项和 $5$ 个负项，没有零项，所以
    \[
        r(f)=10, \qquad r_+=5, \qquad r_-=5.
    \]
\end{solution}
...
所以考虑正定矩阵$A$，其经过合同变换得到单位阵$E_n$，即$C^\mathrm{T}AC = E_n$，两侧取行列式有$\det(C^\mathrm{T})\det(A)\det(C)=\det(C)^2\det(A)=\det(E_n)=1$，所以$\det(A)>0$，即正定矩阵的行列式大于$0$. 而考虑对正定矩阵做限制，即只取前$r$行$r$列，得到的$r$阶子矩阵也必然是正定矩阵，回忆\hyperref[def:子式、主子式、顺序主子式]{顺序主子式的定义}，这启示我们可以通过顺序主子式的符号来判断矩阵的正定性.

\begin{theorem}{Sylvester 判别法}{}
    $n$阶实对称矩阵$A$是正定阵的充分必要条件是$A$的所有顺序主子式的行列式都大于$0$.
\end{theorem}

\begin{proof}
    先证必要性. 设 $A$ 是正定矩阵，记其前 $k$ 行前 $k$ 列构成的顺序主子阵为 $A_k$. 对任意非零向量 $Y=(y_1,\ldots,y_k)^\mathrm{T}$，令
    \[
        X=(y_1,\ldots,y_k,0,\ldots,0)^\mathrm{T},
    \]
    则
    \[
        Y^\mathrm{T}A_kY=X^\mathrm{T}AX>0.
    \]
    因此 $A_k$ 也是正定矩阵. 由前面关于正定矩阵的讨论可知 $\det A_k>0$. 所以 $A$ 的所有顺序主子式都大于 $0$.
...
\begin{solution}
    ...
    \]
    因而正定的充要条件是 $D_2>0,\qquad D_3>0,\qquad D_4>0$. 其中 $D_3>0$ 已经推出 $D_2>0$ 和 $D_4>0$，故最终只需 $3-(t+2)^2>0$. 即
    \[
        -2-\sqrt{3}<t<-2+\sqrt{3}.
    \]
\end{solution}
...
\begin{proof}
    ...
    反之，若 $A$ 正定，则由前面的推论可知 $A$ 合同于单位阵 $E_n$，即存在可逆矩阵 $P$ 使得
    \[
        P^\mathrm{T}AP=E_n.
    \]
    两边同乘以 $P^{-\mathrm{T}}$ 与 $P^{-1}$，得到
    \[
        A=P^{-\mathrm{T}}P^{-1}.
    \]
    令 $C=P^{-1}$，则 $A=C^\mathrm{T}C$.
\end{proof}
...
\begin{proof}
    对 $n$ 作归纳. 当 $n=1$ 时，$A=(a_{11})$ 且 $a_{11}>0$，取 $L=(\sqrt{a_{11}})$ 即可.
    ...
    \[
        S=B-\frac{1}{a_{11}}\alpha\alpha^\mathrm{T}.
    \]
    在证明 Sylvester 判别法时我们已经看到：对任意 $Y\neq 0$，
    \[
        Y^\mathrm{T}SY>0,
    \]
    因而 $S$ 也是正定矩阵. 由归纳假设，存在唯一的下三角矩阵 $L_1$，其对角元均为正，使得
    \[
        S=L_1L_1^\mathrm{T}.
    \]
    令
    ...
    再证唯一性. 设
    \[
        A=LL^\mathrm{T}=\widetilde{L}\widetilde{L}^\mathrm{T},
    \]
    其中 $L,\widetilde{L}$ 都是对角元为正的下三角矩阵. 比较 $(1,1)$ 元得
    \[
        l_{11}^2=\widetilde{l}_{11}^2=a_{11}.
    \]
    又因二者都为正，故 $l_{11}=\widetilde{l}_{11}$. 再比较第一列的其余元素，可得
    \[
        l_{i1}l_{11}=\widetilde{l}_{i1}\widetilde{l}_{11}, \qquad i=2,\ldots,n,
    \]
    从而 $l_{i1}=\widetilde{l}_{i1}$. 去掉第一行第一列后，对右下角 $(n-1)$ 阶块继续同样讨论，递归下去便得到 $L=\widetilde{L}$. 唯一性得证.
\end{proof}
...
\begin{proof}
    先证第 4 条. 由实谱定理，存在正交矩阵 $P$ 使得
    \[
        P^\mathrm{T}AP=\diag(\lambda_1,\ldots,\lambda_n),
    \]
    其中 $\lambda_1,\ldots,\lambda_n$ 是 $A$ 的全部特征值. 对任意 $X=PY$，有
    \[
        X^\mathrm{T}AX=Y^\mathrm{T}(P^\mathrm{T}AP)Y=\lambda_1y_1^2+\cdots+\lambda_ny_n^2.
    \]
    因而 $A$ 半正定当且仅当 $\lambda_i\geqslant 0\enspace(i=1,\ldots,n)$.

    再证第 2 条.

    若 $A$ 半正定，则任取一个主子阵 $B$，对任意非零向量 $Y$，将其嵌入为一个 $n$ 维向量 $X$，则
    \[
        Y^\mathrm{T}BY=X^\mathrm{T}AX\geqslant 0.
    \]
    所以 $B$ 也是半正定矩阵. 由第 4 条，$B$ 的特征值都非负，于是
    \[
        \det B\geqslant 0.
    \]
    因而 $A$ 的各阶主子式都非负.

    反之，设 $A$ 的各阶主子式都非负. 对 $n$ 作归纳.
    ...
    \[
        ...
        =-a_{ij}^2\geqslant 0,
    \]
    故 $a_{ij}=0$. 于是 $A=O$，自然半正定.

    否则存在某个 $a_{ii}>0$. 由于同时交换第 $1$ 行与第 $i$ 行、再交换第 $1$ 列与第 $i$ 列只是在重新排列主子阵，不改变“各阶主子式都非负”这一条件，所以不妨设 $a_{11}>0$. 写成分块形式
    \[
        A=\begin{pmatrix}
            a_{11} & \alpha^\mathrm{T} \\
            \alpha & B
        ...
        S=B-\frac{1}{a_{11}}\alpha\alpha^\mathrm{T}.
    \]
    对任意指标集 $I\subseteq\{2,\ldots,n\}$，记 $S_I$ 为 $S$ 的对应主子阵，$A_I$ 为 $A$ 中由第 $1$ 行、第 $1$ 列以及指标集 $I$ 对应的那些行列组成的主子阵，则
    \[
        \det S_I=\frac{1}{a_{11}}\det A_I\geqslant 0.
    \]
    因而 $S$ 的各阶主子式都非负. 由归纳假设，$S$ 半正定.

    于是对任意 $X=(x_1,Y^\mathrm{T})^\mathrm{T}$，有
    \begin{align*}
        X^\mathrm{T}AX
         & = a_{11}x_1^2+2x_1\alpha^\mathrm{T}Y+Y^\mathrm{T}BY \\
         & = a_{11}\left(x_1+\frac{\alpha^\mathrm{T}Y}{a_{11}}\right)^2+Y^\mathrm{T}SY \\
    ...
    第 1 条是第 2 条的直接推论，因为顺序主子式只是主子式中的一部分.

    第 3 条可由第 4 条立即得到：若 $A$ 半正定，则
    \[
        A=P^\mathrm{T}\diag(\lambda_1,\ldots,\lambda_n)P,
    \]
    其中 $\lambda_i\geqslant 0$. 令
    \[
        C=\diag(\sqrt{\lambda_1},\ldots,\sqrt{\lambda_n})P,
    \]
    则
    \[
        C^\mathrm{T}C=P^\mathrm{T}\diag(\lambda_1,\ldots,\lambda_n)P=A.
    \]
    ...
    设结论对 $n-1$ 阶情形成立. 若 $a_{11}=0$，则由半正定性知对任意 $j\geqslant 2$，
    \[
...
\begin{solution}
    设 $A$ 是秩为 $r$ 的 $n$ 阶实对称矩阵，且 $r<n$. 由实谱定理，存在正交矩阵 $Q$ 使得
    \[
        A=Q^\mathrm{T}\Lambda Q,
    \]
    其中
    \[
        \Lambda=\diag(\lambda_1,\ldots,\lambda_r,0,\ldots,0),
    \]
    且 $\lambda_1,\ldots,\lambda_r\neq 0$.
    ...
    \]
    每个 $D_s$ 都是实对称矩阵，且恰有一个对角元为 $0$，因此
    \[
        r(D_s)=n-1.
    \]
    并且显然有
    \[
        \Lambda=D_1D_2\cdots D_{n-r}.
    \]

    令
    \[
        A_s=Q^\mathrm{T}D_sQ,\qquad s=1,2,\ldots,n-r,
    \]
    则每个 $A_s$ 都是实对称矩阵，并且由于正交相似保持秩不变，有 $r(A_s)=n-1$. 同时
    \[
        A_1A_2\cdots A_{n-r}
        =Q^\mathrm{T}D_1QQ^\mathrm{T}D_2Q\cdots Q^\mathrm{T}D_{n-r}Q
...
从最优化的角度看，二次型与特征值之间的联系尤为重要. 下面补充几条最常用的结论. 在本小节中，若 $A$ 是 $n$ 阶实对称矩阵，我们总把它的特征值按
\[
...
\begin{proof}
    ...
    \[
        \lambda_n\|X\|^2\leqslant X^\mathrm{T}AX\leqslant \lambda_1\|X\|^2.
    \]
    当 $X$ 取对应于 $\lambda_1$ 或 $\lambda_n$ 的特征向量时，上下界分别取到，因此
    \[
        \lambda_1=\max_{X\neq 0}R_A(X), \qquad \lambda_n=\min_{X\neq 0}R_A(X).
    \]
\end{proof}

特别地，实对称矩阵 $A$ 正定当且仅当 $\lambda_n>0$，半正定当且仅当 $\lambda_n\geqslant 0$.
...
\begin{solution}
    ...
    \[
        ...
        \end{pmatrix}.
    \]
    在前面已经算出它的特征值为 $1,6$. 由于约束条件 $x_1^2+x_2^2=1$ 等价于 $\|X\|=1$，故
    \[
        f(X)=X^\mathrm{T}AX=R_A(X).
    \]
    由 Rayleigh 商定理立刻得到
    \[
        \min_{\|X\|=1}f(X)=1,\qquad \max_{\|X\|=1}f(X)=6.
    \]
    当 $X$ 取对应于特征值 $1$ 的单位特征向量 $\dfrac{1}{\sqrt{5}}(-2,1)^\mathrm{T}$ 时取到最小值；当 $X$ 取对应于特征值 $6$ 的单位特征向量 $\dfrac{1}{\sqrt{5}}(1,2)^\mathrm{T}$ 时取到最大值.
\end{solution}

\begin{theorem}{Courant-Fischer 极小极大定理}{}
    设 $A$ 是 $n$ 阶实对称矩阵，特征值按 $\lambda_1\geqslant \cdots \geqslant \lambda_n$ 排列. 则对每个 $k=1,2,\ldots,n$，都有
    \[
...
\begin{proof}
    由实谱定理，存在 $A$ 的一组标准正交特征向量 $\xi_1,\ldots,\xi_n$，满足
    \[
        A\xi_i=\lambda_i\xi_i,\qquad i=1,\ldots,n.
    \]

    先证第一个公式. 对任意 $k$ 维子空间 $U$，因为
    \[
        \dim U+\dim\spa(\xi_k,\xi_{k+1},\ldots,\xi_n)=k+(n-k+1)=n+1,
    \]
    所以
    \[
        U\cap \spa(\xi_k,\xi_{k+1},\ldots,\xi_n)\neq \{\vec{0}\}.
    \]
    取其中非零向量
    \[
        X=a_k\xi_k+\cdots+a_n\xi_n,
    \]
    则
    \[
    ...
    另一方面，取
    \[
        U_0=\spa(\xi_1,\ldots,\xi_k),
    \]
    则对任意非零向量
    \[
        X=a_1\xi_1+\cdots+a_k\xi_k\in U_0
    \]
    都有
    \[
    ...
    再证第二个公式. 对任意维数为 $n-k+1$ 的子空间 $W$，同理由于
    \[
        \dim W+\dim\spa(\xi_1,\ldots,\xi_k)=(n-k+1)+k=n+1,
    \]
    可知
    \[
        W\cap\spa(\xi_1,\ldots,\xi_k)\neq \{\vec{0}\}.
    \]
    取其中非零向量
    \[
        X=a_1\xi_1+\cdots+a_k\xi_k,
    \]
    则
    \[
        R_A(X)=\frac{\lambda_1a_1^2+\cdots+\lambda_ka_k^2}{a_1^2+\cdots+a_k^2}\geqslant \lambda_k.
    ...
    因而
    \[
        \max_{0\neq X\in W}R_A(X)\geqslant \lambda_k.
    \]
    对所有这类 $W$ 取最小，得到
    \[
    ...
    另一方面，取
    \[
        W_0=\spa(\xi_k,\xi_{k+1},\ldots,\xi_n),
    \]
    则对任意非零 $X\in W_0$，都有 $R_A(X)\leqslant \lambda_k$，故
    \[
        \max_{0\neq X\in W_0}R_A(X)\leqslant \lambda_k.
    \]
    再取 $X=\xi_k$，便知这一最大值恰为 $\lambda_k$. 因此
    \[
        \min_{\dim W=n-k+1}\ \max_{0\neq X\in W}R_A(X)=\lambda_k.
    \]
\end{proof}

\begin{theorem}{Cauchy 交错定理}{}
    设 $A$ 是 $n$ 阶实对称矩阵，特征值为
    \[
        \lambda_1\geqslant \lambda_2\geqslant \cdots \geqslant \lambda_n.
    \]
    设 $B$ 是 $A$ 的一个 $m$ 阶主子阵，特征值为
    \[
        \mu_1\geqslant \mu_2\geqslant \cdots \geqslant \mu_m.
    \]
    则对每个 $i=1,2,\ldots,m$，都有
    \[
...
\begin{proof}
    不妨设 $B$ 对应于坐标子空间 $V\subseteq \mathbf{R}^n$；也就是说，$B$ 由 $A$ 删去若干行和对应列得到. 对任意 $Y\in\mathbf{R}^m$，将它按对应位置嵌入为 $V$ 中的向量 $\widetilde{Y}$，则
    \[
        Y^\mathrm{T}BY=\widetilde{Y}^\mathrm{T}A\widetilde{Y}, \qquad Y^\mathrm{T}Y=\widetilde{Y}^\mathrm{T}\widetilde{Y}.
    \]
    因而 $B$ 在 $Y$ 上的 Rayleigh 商恰等于 $A$ 在 $\widetilde{Y}$ 上的 Rayleigh 商.
    ...
    \]
    而由 Courant-Fischer 定理，
    \[
        \lambda_{n-m+i}=\min_{\dim W=m-i+1}\ \max_{0\neq X\in W}R_A(X).
    \]
    由于后者是在所有维数为 $m-i+1$ 的子空间上取最小，前者只是在其中一部分子空间上取最小，所以
    \[
        \lambda_{n-m+i}\leqslant \mu_i.
    \]
    两式合并即得
    \[
        \lambda_i\geqslant \mu_i\geqslant \lambda_{n-m+i}.
...
\begin{theorem}{Weyl 不等式}{}
    设 $A,B$ 是 $n$ 阶实对称矩阵，它们的特征值分别按降序排列为
    \[
        \alpha_1\geqslant \cdots \geqslant \alpha_n,\qquad \beta_1\geqslant \cdots \geqslant \beta_n.
    \]
    设 $A+B$ 的特征值按降序排列为
    \[
        \gamma_1\geqslant \cdots \geqslant \gamma_n.
    \]
    则
    \begin{enumerate}
        \item 若 $i+j-1\leqslant n$，则
              \[
                  \gamma_{i+j-1}\leqslant \alpha_i+\beta_j;
              \]
        \item 若 $i+j-n\geqslant 1$，则
              \[
                  \gamma_{i+j-n}\geqslant \alpha_i+\beta_j.
              \]
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    先证第 1 条. 令
    \[
        U=\spa(\xi_i,\xi_{i+1},\ldots,\xi_n),\qquad V=\spa(\eta_j,\eta_{j+1},\ldots,\eta_n).
    \]
    则
    \[
        \dim U=n-i+1,\qquad \dim V=n-j+1,
    \]
    从而
    \[
        \dim(U\cap V)\geqslant (n-i+1)+(n-j+1)-n=n-i-j+2.
    \]
    当 $i+j-1\leqslant n$ 时，上式右端正好是
    \[
        n-(i+j-1)+1.
    \]
    取 $U\cap V$ 的一个维数为 $n-(i+j-1)+1$ 的子空间 $W$. 对任意非零 $X\in W$，都有
    \[
        R_A(X)\leqslant \alpha_i,\qquad R_B(X)\leqslant \beta_j,
    \]
    故
    \[
    ...
    再证第 2 条. 令
    \[
        U=\spa(\xi_1,\ldots,\xi_i),\qquad V=\spa(\eta_1,\ldots,\eta_j).
    \]
    则
    \[
        \dim U=i,\qquad \dim V=j,
    \]
    从而
    \[
        \dim(U\cap V)\geqslant i+j-n.
    \]
    当 $i+j-n\geqslant 1$ 时，可取 $U\cap V$ 的一个维数为 $i+j-n$ 的子空间 $W$. 对任意非零 $X\in W$，都有
    \[
        R_A(X)\geqslant \alpha_i,\qquad R_B(X)\geqslant \beta_j,
    \]
    故
    \[
        R_{A+B}(X)=R_A(X)+R_B(X)\geqslant \alpha_i+\beta_j.
    \]
    因而
    \[
        \min_{0\neq X\in W}R_{A+B}(X)\geqslant \alpha_i+\beta_j.
    \]
    再由 Courant-Fischer 定理，
    \[
        \gamma_{i+j-n}=\max_{\dim W'=i+j-n}\ \min_{0\neq X\in W'}R_{A+B}(X)\geqslant \alpha_i+\beta_j.
    \]
    第 2 条得证.
\end{proof}
...
\begin{proof}
    在 Weyl 不等式中分别取 $(i,j)=(k,n)$ 与 $(i,j)=(k,1)$ 即可.
\end{proof}
...
\begin{proof}
    记 $C=B-A$. 由假设知 $C$ 半正定，所以它的最小特征值满足 $\lambda_n(C)\geqslant 0$. 又 $B=A+C$，故由上一个推论可得
    \[
        \lambda_k(B)\geqslant \lambda_k(A)+\lambda_n(C)\geqslant \lambda_k(A).
    \]
\end{proof}

\begin{summary}
    本讲围绕实数域上二次型的标准化展开，此时二次型与对称矩阵一一对应；求二次型的标准形，本质上就是把相伴矩阵作合同对角化. 这可以通过初等变换、配方法或正交变换来完成.

    在实数域上，合同对角化进一步导向相合规范形与惯性定理：实对称矩阵的正、负、零惯性指数是合同不变量. 由此可以把正定、半正定等保号性质完全转化为规范形、特征值与矩阵分解的条件，并得到 Sylvester 判别法、特征值判别法以及 Cholesky 分解等一系列常用工具.

    在应用部分，一方面可以借助相抵、相似与合同标准形处理矩阵分解问题；另一方面，Rayleigh 商、Courant-Fischer 极小极大定理、Cauchy 交错定理与 Weyl 不等式把二次型和特征值估计紧密联系起来，这正是后续微积分、最优化和数值分析中最常见的线性代数基础.
\end{summary}
...
\begin{exercise}
    ...
    \begin{exgroup}
        ...
        \[
            ...
            \end{pmatrix}.
        \]
        不直接求偏导，利用 Rayleigh 商定理求二次型 $2x_1^2+5x_2^2+4x_1x_2$ 在单位圆 $x_1^2+x_2^2=1$ 上的最大值与最小值.
    \end{exgroup}
    ...
    \begin{exgroup}
        ...
        \item 设 $A$ 是 $n$ 阶实对称矩阵，$B$ 是 $A$ 的一个 $m$ 阶主子阵. 证明 Cauchy 交错定理
        \[
            \lambda_i(A)\geqslant \lambda_i(B)\geqslant \lambda_{n-m+i}(A), \qquad i=1,\ldots,m.
        \]
        并由此推出：若 $A$ 正定，则任一主子阵也正定.
