...
\begin{definition}
    如果一个向量组的每个向量范数都是 1 且与其他向量正交则称这个向量组是\keyterm{标准正交}[orthonormal]（规范正交）的.
\end{definition}
...
\begin{theorem}[\keyterm{Schur 定理}] \label{thm:23:Schur}
...
\begin{definition}
    设 $ U $ 是 $ V $ 的子集，则 $ U $ 的\keyterm{正交补}[orthogonal complement]（记作 $ U^{\perp } $）是由 $ V $ 中与 $ U $ 的每个向量都正交的那些向量组成的集合：
