...
\begin{exercise}
    ...
    \begin{exgroup}
        ...
        \]
        \begin{enumerate}
            \item 把 $\operatorname{End}_K(L)$ 看作左 $L$-线性空间，证明
            \[
        ...
        \end{enumerate}
        \item 本题补充有限投射模和平坦模的基本工具. 设 $A$ 是交换环，$M$ 是 $A$-模. 若存在 $A$-模 $N$ 与非负整数 $r$，使得
        \[
            M\oplus N\cong A^r,
        \]
        则称 $M$ 是一个\term{有限投射（finite projective）}$A$-模；换言之，有限投射模就是有限自由模的直和因子. 若有限个元素 $x_i\in M$ 与 $A$-线性映射 $\varphi_i\in\operatorname{Hom}_A(M,A)$ 满足对任意 $x\in M$ 都有 $x=\sum_i x_i\varphi_i(x)$，则称它们构成 $M$ 的一组有限\term{对偶基（dual basis）}.
        \begin{enumerate}
            ...
            \item 反过来，若 $M$ 是有限投射 $A$-模，则证明存在满足上一问条件的 $x_i$ 与 $\varphi_i$. 因此，有限投射模等价于存在这样一组有限\term{对偶基（dual basis）}.
