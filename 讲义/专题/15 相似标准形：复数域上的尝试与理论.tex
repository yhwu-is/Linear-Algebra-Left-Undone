...
上述定理的证明蕴含着一个重要的结论：如果一个线性变换可对角化，那么对应于对角矩阵的基中每一个向量必然是特征向量，因此我们很容易想到可对角化的下一个等价条件就是线性变换有一组由特征向量构成的基. 当然这只是一个观察，基于这一观察我们可以进一步扩展，得到下面这一关于线性变换可对角化等价条件的核心定理：
...
\begin{solution}
    \begin{enumerate}
        \item 我们知道，$r(A)\leqslant\min{\{r(\alpha),r(\beta)\}}=1$，并且$A\neq O$因此$r(A)>0$，故$A$的秩为1. 而$n>1$，因此$A$一定不可逆，故0一定是$A$的特征值，且对应的特征子空间维数为$AX=0$的解空间维数，即为$n-1$.
              ...
              由此我们知道$A$最多有两个特征值，因为0的代数重数（即作为$n$次特征多项式的零点次数）必然大于等于其几何重数$n-1$，当其代数重数为$n-1$时可能还有一个一重特征值. 我们利用特征值之和等于$A$的迹来找出可能的第二个特征值. 我们设$\alpha=(a_1,a_2,\ldots,a_n)^\mathrm{T},\beta=(b_1,b_2,\ldots,b_n)^\mathrm{T}$，则$A=\alpha\beta^\mathrm{T}=\begin{pmatrix}
...
\begin{proof}
    \begin{enumerate}
        \item 由题意$A^2-2A=O$，由\autoref{ex:多项式零点为特征值}，$A$的特征值是方程$\lambda^2-2\lambda=0$的解，即$\lambda_1=0,\lambda_2=2$.
...
我们仍然循着\autoref{thm:不变子空间与分块对角矩阵} 提供的范式寻找相似标准形，即我们的目标仍然是寻找线性空间的一个不变子空间分解。我们知道线性变换不可对角化实际上是因为它没有足够多的线性无关的特征向量，也即特征子空间直和后比原空间略小，因此基于特征子空间这一不变子空间的直和分解并不对所有线性变换成立，于是我们很自然地会有一个想法，即我们能否将线性变换的特征子空间进行某种扩张，使得扩张后的特征子空间的直和能够张成整个线性空间呢？如果这样可行至少可以得到一个分块对角矩阵，这样的矩阵大概在我们可接受的相似标准形内.

事实上，注意到$\sigma$在特征值$\lambda$下的特征子空间就是$\ker(\lambda I-\sigma)$，并且我们曾经讲过核空间可以扩张的性质（\autoref{thm:核空间性质}），这一想法便有了成功的可能性. 为了讨论的方便我们重新叙述一下核空间扩张的性质：
\begin{theorem}{}{}
...
因此，如果考虑$\ker(\sigma-\lambda I)^n$，其中$n$是$\sigma$对应线性空间的维数，那么我们可以获得比特征子空间更大的核空间，这样扩张后的核空间的直和是否可以张成整个原空间呢？我们可以提前给出答案：可以，这便是我们下一小节中要证明的主要定理，在此之前，我们需要首先给这一扩张后的特征子空间一个新的名字.
...
类似于特征值与特征向量的讨论，对于矩阵我们也可以讨论广义特征向量，实际上就是满足存在正整数$j$使得$(A-\lambda E)^jv=0$的向量$v$，这样的向量称为矩阵$A$对应于特征值$\lambda$的广义特征向量. 对于$n$阶矩阵，$A^kX=0$的解空间与核空间的性质也是完全一致的，因此广义特征子空间就等于$\ker(A-\lambda E)^n$. 为了我们叙述的简洁性，后面矩阵的等价表述我们就不再给出，读者完全可以自行推导. 另一个关键的问题是线性变换与其矩阵表示的广义特征向量的关系，事实上完全类似于特征向量之间的关系，即坐标对应的关系，证明也与特征向量的证明类似，这里也不再赘述.
...
\begin{theorem}{}{广义特征性质}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item 每个$G_{\lambda_j}$在$\sigma$下都是不变的；
...
\begin{proof}
    \begin{enumerate}
        \item $\forall v\in G_{\lambda_j}$，根据$G_{\lambda_j}$定义可知$(\sigma-\lambda_j I)^nv=0$，因此限制在$G_{\lambda_j}$上，$((\sigma-\lambda_j I)\vert_{G_{\lambda_j}})^n$将所有向量化零，是零线性变换，故$\sigma-\lambda_j I$是幂零的.

        \item 设$v\in G_{\lambda_j}$，根据$G_{\lambda_j}$定义可知这等价于存在正整数$k$使得$(\sigma-\lambda_j I)^kv=0$. 考虑$\sigma(v)$，我们有
              \[(\sigma-\lambda_j I)^k(\sigma(v))=\sigma((\sigma-\lambda_j I)^k(v))=0,\]
              其中$\sigma$与$(\sigma-\lambda_j I)^k$可交换是来源于\autoref{ex:矩阵多项式可交换}，因此$\sigma(v)\in G_{\lambda_j}$.
        ...
        \item 对$V$的维数$n$归纳来证明. 当维数为1时，结论是显然的. 假设对任意小于$n$的维数结论都成立，现在我们对维数为$n$的情况证明. 由于是复向量空间，则必然有一个特征值$\lambda_1$，对应的广义特征子空间$G(\lambda_1,\sigma)$（使用这一记号是为了防止后续线性变换不同引起歧义）的维数不小于1. 由\autoref{thm:核空间扩张分解}，我们有
              \[V=\ker(\sigma-\lambda_1 I)^n\oplus\im(\sigma-\lambda_1 I)^n,\]
              即
              \begin{equation} \label{eq:16:广义特征子空间分解}
                  V=G(\lambda_1,\sigma)\oplus W,
              \end{equation}
              其中$W=\im(\sigma-\lambda_1 I)^n$. 由\autoref{ex:多项式不变子空间} 可知$W$是$\sigma$的不变子空间，又根据\autoref{eq:16:广义特征子空间分解} 可知$\dim W<n$，因此我们可以考虑对$\sigma\vert_W$使用归纳假设，有
              \[W=G(\lambda_2,\sigma\vert_W)\oplus\cdots\oplus G(\lambda_m,\sigma\vert_W),\]
              因此为了完成证明，我们只需证明$G(\lambda_j,\sigma)=G(\lambda_j,\sigma\vert_W),\enspace j=2,\ldots,m$，而根据广义特征子空间的定义，包含关系$G(\lambda_j,\sigma\vert_W)\subset G(\lambda_j,\sigma)$是显然的，因此我们只需证明另一半包含.

              设$v\in G(\lambda_j,\sigma)\subset V$，由\autoref{eq:16:广义特征子空间分解} 可知$v=v_1+w$，其中$v_1\in G(\lambda_1,\sigma),w\in W$. 而由归纳假设，$w=v_2+\cdots+v_m$，其中$v_j\in G(\lambda_j,\sigma\vert_W)$，因此$v=v_1+v_2+\cdots+v_m$，改写一下得到
              \[v_1+\cdots+(v_j-v)+\cdots+v_m=0,\]
              而我们根据上一条证明知道对应于不同特征值的广义特征向量线性无关，而$v_j-v\in G(\lambda_j,\sigma)$，因此只能有$v_1=\cdots=v_{j-1}=v_{j+1}=\cdots=v_m=0$，且$v=v_j$. 特别地有$v_1=0$，因此$v=w=v_j\in G(\lambda_j,\sigma\vert_W)$，故包含得证.
...
\begin{solution}
    ...
    \[ \begin{pmatrix}
        ...
        0 & 0 & 7
    \end{pmatrix}, \]
    则 $\sigma$的特征值为 $\lambda_1 = 6,\lambda_2 = 7$. 解方程可以得到两个广义特征子空间 $G(\lambda_1,\sigma) = \spa\left((1,0,0),\left(0,\dfrac{1}{3},0\right)\right)$，$G(\lambda_2,\sigma)=\spa((10,2,1))$.
...
当然，有的读者或许认为到目前为止我们对于复数域上线性变换与矩阵的标准形讨论已经进入尾声，毕竟我们已经按照\autoref{thm:不变子空间与分块对角矩阵} 提供的范式找到了对任意线性变换都存在的不变子空间分解，得到了分块对角矩阵的标准形. 然而分块对角矩阵有时候可能会遇到分块很大的情况，或者矩阵本身只有一个特征值因此整个矩阵就是唯一的分块，这时矩阵并不一定足够简单，所以在后续我们还会通过更加精细的分解来实现更漂亮的矩阵标准形. 在下一节中我们给出一个对于这一问题不成熟但对于研究复数域上不变子空间性质很重要的解决方案，即给出上三角矩阵这一类任意复数域上线性变换和矩阵都可以得到的矩阵标准形.
...
\begin{proof}
$1 \implies 2$：设$\sigma$关于$v_1,\ldots,v_n$的矩阵$A=(a_{ij})_{n\times n}$是上三角的，即$a_{ij}=0,\forall i>j$. 则对于$\forall j \in \{1,\ldots,n\}$，有$\sigma(v_j)=\displaystyle\sum_{i=1}^{n}a_{ij}v_i=\displaystyle\sum_{i=1}^{j}a_{ij}v_i\in  \spa(v_1,\ldots,v_j)$.
...
由2知对于$\forall i \in \{1,\ldots,j\}$，有$\sigma(v_i) \in \spa(v_1,\ldots,v_i) \subseteq \spa(v_1,\ldots,v_j)$，从而知$\sigma(v) \in \spa(v_1,\ldots,v_j)$，即$\spa(v_1,\ldots,v_j)$在$\sigma$下不变.

$3 \implies 1$：设$\sigma$关于$v_1,\ldots,v_n$的矩阵为$A=(a_{ij})_{n\times n}$，对于$\forall j \in \{1,\ldots,n\}$，$\sigma(v_j)=\displaystyle\sum_{i=1}^{n}a_{ij}v_i\in \spa(v_1,\ldots,v_j)$，从而有$a_{j+1,j}=\cdots=a_{n,j}=0$，即$\forall i>j,a_{ij}=0$. 故$A$为上三角矩阵.
\end{proof}
...
我们提供三种证明方法，帮助读者更好地运用已经得到的结论理解这个问题：

\begin{proof}
    以下均对维数$n=\dim V$使用归纳法.
    \begin{enumerate}
        \item[法一:] 采用第二数学归纳法，$n=1$时结论显然. 假设小于$n$维的复向量空间上的线性变换都可以找到一组基使得其关于这组基有上三角矩阵，现在考虑$n$维复向量空间，由\autoref{thm:复数域上的特征值}，设$\lambda$是$\sigma$的一个特征值，考虑$U=\im(\sigma-\lambda I)$，容易验证$U$是$\sigma$的比$V$维数更小的不变子空间，用归纳假设，$U$存在一组基$u_1,\ldots,u_m$使得$\sigma\vert_U$有上三角矩阵. 由\autoref{thm:上三角矩阵等价条件}，对于$\forall j\in\{1,\ldots,m\}$都有
        \[\sigma(u_j)=\sigma\vert_U(u_j)\in\spa(u_1,\ldots,u_j),\]
        现在我们得到了一部分的基使得限制映射有上三角矩阵，在此基础上要使得全空间上的映射有上三角矩阵，不难想到要进行基扩充. 将上面取到的基扩充为$u_1,\ldots,u_m,v_1,\ldots,v_k$，对$\forall j\in\{1,\ldots,k\}$，$\sigma(v_j)=(\sigma-\lambda I)(v_j)+\lambda v_j$，其中$(\sigma-\lambda I)(v_j)\in U=\spa(u_1,\ldots,u_m)$，因此$\sigma(v_j)\in\spa(u_1,\ldots,u_m,v_1,\ldots,v_j)$，根据\autoref{thm:上三角矩阵等价条件} 可知$\sigma$关于这组基有上三角矩阵. 进而由归纳法知结论成立.

        \item[法二:] 根据相似的定义，\autoref{thm:上三角矩阵存在} 的结论与``任意$n$阶复矩阵一定相似于一个上三角矩阵''是等同的，因此我们考虑矩阵的理论——使用分块矩阵结合数学归纳法的方法进行证明.

        $n=1$时结论显然，因为任意一阶矩阵本身就是上三角矩阵. 假设$n-1$阶复矩阵都可以相似于上三角矩阵，设$A$为$n$阶复矩阵，任取$A$的一个复特征值$\lambda$，设$\alpha_1$为$A$对应于$\lambda$的特征向量. 我们把$\alpha_1$扩充为$\mathbf{C}^n$的一组基，记为$\alpha_1,\alpha_2,\ldots,\alpha_n$，记$P_1=(\alpha_1,\alpha_2,\ldots,\alpha_n)$，则$P_1$可逆，且\[P_1^{-1}AP_1=\begin{pmatrix}
            \lambda & \alpha' \\ O & A_{n-1}
        \end{pmatrix},\]
        对$n-1$阶矩阵$A_{n-1}$应用归纳假设，存在可逆矩阵$P_2$使得$P_2^{-1}A_{n-1}P_2$为上三角矩阵，取$P=P_1\begin{pmatrix}
            1 & O \\ O & P_2
        \end{pmatrix}$，直接有\[P^{-1}AP=\begin{pmatrix}
            \lambda & \alpha'P_2 \\ O & P_2^{-1}A_{n-1}P_2
        \end{pmatrix}\]
        为上三角矩阵，因此$n$阶复矩阵一定相似于上三角矩阵.

        \item[法三:] 使用数学归纳法，$n=1$时结论显然. 假设结论对$n-1$维的情形成立，现在考虑$n$维复向量空间，由\autoref{thm:复数域上的特征值}，设$\lambda$是$\sigma$的一个特征值，$v_1$是对应的特征向量，$U=\spa(v_1)$，这是一个1维不变子空间，而归纳假设需要我们处理$n-1$阶的子空间，这使我们联想到商空间，因为$V/U$是一个很自然的$n-1$维复向量空间.

        考虑商映射$\sigma/U\in\mathcal{L}(V/U)$，根据归纳假设，存在$V/U$的一组基使得$\sigma/U$关于这组基$v_2+U,\ldots,v_n+U$有上三角矩阵，根据\autoref{thm:上三角矩阵等价条件}，对每个$j\in\{2,\ldots,n\}$都有\[(\sigma/U)(v_j+U)=\sigma(v_j)+U\in\spa(v_1+U,\ldots,v_j+U),\]
        因此$\sigma(v_j)\in\spa(v_1,\ldots,v_j)$，因此结合$v_1$是特征向量的性质，我们可以得到一组基$v_1,\ldots,v_n$使得$\sigma$关于这组基有上三角矩阵.
    \end{enumerate}
\end{proof}

\autoref{thm:上三角矩阵存在} 证明的法二的意义在于，这一证明给了我们一个求解矩阵上三角化的方法（与对角化等类似，线性变换的上三角化依赖于矩阵上三角化，因此这里只讨论矩阵的情况）. 根据上面的证明，我们只需任意挑选$n$阶矩阵的一个特征值和一个对应的特征向量，然后问题就转化为求解$n-1$阶矩阵的上三角化问题，那么我们继续求出$n-1$阶矩阵的一个特征值和一个对应的特征向量，依次类推直到一阶的情况. 我们给出下面的例子供读者运用这一方法：
\begin{example}{}{}
...
\begin{solution}
    ...
    \[
        ...
        \end{pmatrix}.
    \]
    下面按照\autoref{thm:上三角矩阵存在} 证明的法二逐步构造相似变换矩阵. 注意到 $A(1,1,0,0)^\mathrm{T} = 0$，因此 $0$ 是 $A$ 的一个特征值，对应的特征向量可取 $\alpha_1=(1,1,0,0)^\mathrm{T}$. 令 $P_1 = (\alpha_1,e_2,e_3,e_4)$，则
...
综合\autoref{thm:上三角矩阵等价条件} 的 3 和\autoref{thm:上三角矩阵存在}，我们可以观察到不变子空间的另一个有趣的性质：
\begin{corollary}{}{}
    设$V$是$n$维复向量空间. $\sigma\in \mathcal{L}(V)$，则对任意的正整数$r\enspace(1\leqslant r\leqslant n)$，$\sigma$有$r$维不变子空间.
\end{corollary}
这是因为任意复向量空间上的线性变换都存在一组基使得矩阵表示为上三角矩阵，而存在上三角矩阵的其中一个充要条件是存在任意维数的不变子空间，因此任意复向量空间上的线性变换都存在任意维数的不变子空间. 如果更进一步地利用\autoref{thm:上三角矩阵等价条件} 的第三点，我们有如下推论：
\begin{corollary}{}{}
...
最后，根据\autoref{thm:上三角矩阵存在}，我们可以综合分块对角矩阵的存在性，将每个广义特征子空间的基取成上三角矩阵的基，然后将这些基拼在一起，就可以得到一个每个分块都是上三角矩阵的分块对角矩阵，显然比之前单纯的分块对角矩阵和上三角矩阵都要简单：
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 这一结论的证明与\autoref{thm:上三角矩阵存在} 类似，也有矩阵和商空间两个版本，限于篇幅这里只给出商空间版本的证明.

        我们对维数$n$归纳证明. $n=1$时，结论显然成立. 假设命题对$n-1$维成立. 由于$\sigma\tau=\tau\sigma$，故$\exists \alpha\in V$，使得$\alpha$同时为$\sigma$和$\tau$的特征向量. 设$U=\spa(\alpha)$，则$U$是$\sigma$和$\tau$的不变子空间. 考虑商映射$\sigma/U,\tau/U$，对于$\forall v+U\in V/U$，有\[((\sigma/U)(\tau/U))(v+U)=\sigma(\tau(v))+U=\tau(\sigma(v))+U=((\tau/U)(\sigma/U))(v+U),\]
        即$(\sigma/U)(\tau/U)=(\tau/U)(\sigma/U)$. 由$\dim V/U=n-1$，结合归纳假设可知：存在$V/U$的一组基$v_1+U,\ldots,v_{n-1}+U$，使得$\sigma/U$和$\tau/U$在这组基下都是上三角矩阵.

        从而$(\sigma/U)(v_j+U)=\sigma(v_j)+U \in \spa(v_1+U,\ldots,v_j+U)$，故$\sigma(v_j)\in \spa(\alpha,v_1,\ldots,v_j),\forall j=1,2,\ldots,n-1$.
...
我们在前文已经基本完成了对复数域上线性变换的相似标准形基本理论的讨论，这一理论包括对角标准形的充要条件，任意线性映射的广义特征子空间分解与分块对角矩阵标准形，以及上三角矩阵的等价条件与存在性，接下来我们将利用这些理论来解决一些具体的问题.
...
实际上，幂等矩阵还有很多其他的性质，我们可以回到线性变换的角度去理解这一矩阵，不仅仅是将上述定理换成线性变换的描述进行讨论（因为根据线性映射矩阵表示很容易知道，幂等线性变换在任意一组基下的矩阵表示都是幂等矩阵），我们还可以讨论其与投影变换的等价性. 我们首先给出投影变换的定义，读者不难验证如下定义的投影变换确实是一个线性变换：

\begin{definition}{}{}
    设$V$是数域$\mathbf{F}$上的线性空间，$U,W$是$V$的子空间且$V=U\oplus W$. 定义 $V$ 到 $U$ 上的\term{投影变换}为如下线性变换$\pi\in \mathcal{L}(V)$：对任意$v\in V$，由直和分解唯一地写成$v=u+w,\enspace u\in U,\enspace w\in W$，然后令$\pi(v)=u$.
\end{definition}

\begin{theorem}{}{幂等变换等价于投影变换}
    设$V$是数域$\mathbf{F}$上的线性空间，$\sigma\in \mathcal{L}(V)$. 则$\sigma$幂等当且仅当$\sigma$是一个投影变换. 更具体地，$\sigma$ 是 $V$ 到 $\im \sigma$ 上的投影变换.
\end{theorem}

\begin{proof}
    必要性：若$\sigma$是沿 $V$ 到 $U$ 上的投影变换，则对任意$v=u+w\enspace(u\in U,\enspace w\in W)$，有$\sigma(v)=u$，从而$\sigma^2(v)=\sigma(u)=u=\sigma(v)$，故$\sigma^2=\sigma$.

    充分性：若 $\sigma^2 = \sigma$，根据\autoref{sec:像与核的进一步讨论}的讨论，有 $V = \im \sigma \oplus \ker \sigma$. 因此对任意 $v = u + w \enspace(u \in \im \sigma, \enspace w \in \ker \sigma)$，由 $u \in \im \sigma$可设 $u = \sigma(y)$，于是$\sigma(u)=\sigma^2(y)=\sigma(y)=u$，并且$\sigma(w)=0$，故$\sigma(v)=\sigma(u+w)=u$. 这表明$\sigma$正是 $V$ 到 $\im \sigma$ 上的投影变换.
\end{proof}

这一结论的直观意义是：幂等变换做一次之后再做第二次不会再发生新的变化，因此它一定是在``保留某个分量，消去另一个分量''. 在分解$V=\im \sigma\oplus\ker \sigma$中，$\im \sigma$中的向量是已经被保留下来的部分，而$\ker \sigma$中的向量则是在投影过程中被消去的部分. 这也解释了为什么投影变换天然满足幂等性：一个向量一旦已经落在目标子空间中，再投影一次就不会改变.

进一步地，在实数域上，对称幂等变换等价于正交投影变换，这将在后续讲解内积空间时说明（\autoref{thm:对称幂等变换与正交投影的关系}）. 最后给出一个技巧性较强的题目，读者可以在此题中体会``幂等''这一性质的特点：
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 考虑矩阵版本，结合第一点可知$N\pm I$的特征值全为1（或-1），因此一定是可逆的.
    \end{enumerate}
\end{proof}
...
\begin{proof}
    \begin{enumerate}
        \item 若$A$可逆，在$AB-BA=A$两边同时左乘$A^{-1}$，则$B-A^{-1}BA=E$. 取迹得$\tr(B)-\tr(A^{-1}BA)=\tr(E)$，又因为$\tr(A^{-1}BA)=\tr(A^{-1}AB)=\tr(B)$，因此$\tr(E)=0$，这与$\tr(E)=n$矛盾. 故$A$不可逆.
        \item 考虑从$\mathbf{M}_n(F)$到自身的映射$\sigma:\sigma(X)=XB-BX$，则$\sigma(A)=A$. 容易验证$\sigma$是线性映射. 归纳容易证明：$\sigma(A^k)=kA^k,\forall k\in \mathbf{N}^*$

        假设$A$不是幂零的，则对$\forall k \in \mathbf{N}^*$，有$A^k \neq 0,\sigma(A^k)=kA^k$. 这表明任意正整数$k$都是$\sigma$的特征值，但$\sigma$最多只能有$r(\mathbf{M}_n(F))=n^2$个互异的特征值（回忆\autoref{thm:特征向量的基本性质}），矛盾. 故$A$是幂零矩阵.
    \end{enumerate}
\end{proof}

\subsection{平方根问题} % TODO: 感觉这里还能加内容，先挖个坑（
...
\begin{proof}
    \begin{enumerate}
        ...
        \item 设$\lambda_1,\ldots,\lambda_m$是$\sigma$所有互不相同的本征值，由\autoref{thm:广义特征性质}，对每个$j\in\{1,\ldots,m\}$都存在一个幂零算子$N_j\in \mathcal{L}(G_{\lambda_j})$使得$\sigma\vert_{G_{\lambda_j}}=\lambda_j I+N_j$. 因为$\sigma$可逆，所以$\lambda_j \neq 0$，于是有$\sigma\vert_{G_{\lambda_j}}=\lambda_j(I+\dfrac{N_j}{\lambda_j})$. 显然$\dfrac{N_j}{\lambda_j}$也是幂零的，故$I+\dfrac{N_j}{\lambda_j}$有平方根，从而$\sigma\vert_{G_{\lambda_j}}$有平方根，取一个记为$R_j$.

        对于$\forall v\in V$，$v$可以唯一地写成$v=u_1+\cdots+u_m$，其中$u_j\in G_{\lambda_j}$. 我们定义算子$R\in\mathcal{L}(V)\colon Rv=R_1u_1+\cdots+R_mu_m$. 不难验证我们定义的$R$即为$\sigma$的平方根.
    \end{enumerate}
\end{proof}
...
在\autoref{thm:特征值与不变子空间} 中我们谈到了``复化''的思想，接下来我们将这一思想用严格的定义写出来. 先从实线性空间出发定义复化的概念：

\begin{definition}{}{}
    设$V$是实线性空间，定义$V$的复化为$V\times V$，记为$V_{\mathbf{C}}$，其元素$(u,v)$我们记为$u+v\i$，其中$u,v\in V$，并定义$V_{\mathbf{C}}$上的加法和数乘如下：
    \begin{itemize}
        \item 定义$V_{\mathbf{C}}$上的加法为$(u_1+v_1\i)+(u_2+v_2\i)=(u_1+u_2)+(v_1+v_2)\i$，其中$u_1,u_2,v_1,v_2\in V$；
        \item 定义$V_{\mathbf{C}}$上的数乘为$(a+b\i)(u+v\i)=(au-bv)+(av+bu)\i$，其中$a,b\in\mathbf{R},\enspace u,v\in V$.
    \end{itemize}
\end{definition}

读者可以自行验证基于$V$扩展而来的$V_{\mathbf{C}}$是复线性空间，并且基于我们对``复化''的直觉，下面的结论也是容易得到的（证明我们留作习题）：

\begin{example}{}{复化空间的维数}
    设$V$是实线性空间，若$v_1,\ldots,v_n$是$V$（作为实线性空间）的一组基，那么它也是$V_{\mathbf{C}}$（作为复线性空间）的一组基，从而$\dim V_{\mathbf{C}}=\dim V$.
\end{example}

一个帮助读者理解所谓``对复化的直觉''的例子是，$\mathbf{R}^n(\mathbf{R})$的复化为$\mathbf{C}^n(\mathbf{C})$，这表明复化过程并没有改变空间的维数，而是将实数域上的向量扩展到了复数域上.

基于实线性空间的复化，我们可以进一步定义实线性空间上线性变换的复化：

\begin{definition}{}{}
    设$V$是实线性空间，$\sigma\in \mathcal{L}(V)$，定义$\sigma$的复化为$\sigma_{\mathbf{C}}\in \mathcal{L}(V_{\mathbf{C}})$，其作用为$\sigma_{\mathbf{C}}(u+v\i)=\sigma(u)+\sigma(v)\i$，其中$u,v\in V$.
\end{definition}

请读者自行验证，$\sigma_{\mathbf{C}}$是良定义的且确实属于$\mathcal{L}(V_{\mathbf{C}})$. 并且基于\autoref{ex:复化空间的维数}，我们有（证明同样留作习题）：

\begin{example}{}{复化线性变换的矩阵表示}
    设$V$是实线性空间，$\sigma\in \mathcal{L}(V)$，若$A$是$\sigma$在某一组基下的矩阵表示，则$A$也是$\sigma_{\mathbf{C}}$在同一组基下的矩阵表示.
\end{example}

在此基础上，我们可以讨论复化的特征值与特征向量的性质：

\begin{theorem}{}{复化线性变换的特征值与特征向量}
    设$V$是实线性空间，$\sigma\in \mathcal{L}(V)$，
    \begin{enumerate}
        \item 若$\lambda\in\mathbf{R}$，则$\lambda$是$\sigma_{\mathbf{C}}$的特征值的充分必要条件是$\lambda$是$\sigma$的特征值；
        \item 若$\lambda=a+b\i(a,b\in\mathbf{R})$，则$\lambda$是$\sigma_{\mathbf{C}}$的特征值的充分必要条件是存在不全为零的$\alpha,\beta\in V$使得
        \[\sigma(\alpha)=a\alpha-b\beta,\enspace \sigma(\beta)=b\alpha+a\beta.\]
        此时$\alpha+\beta\i$是$\sigma_{\mathbf{C}}$属于$\lambda$的特征向量.
        \item 若$\lambda=a+b\i \in\mathbf{C}$，$\alpha,\beta\in V$，$j$为非负整数，那么
        \[(\sigma_{\mathbf{C}}-\lambda I)^j(\alpha+\beta\i)=0 \iff (\sigma_{\mathbf{C}}-\overline{\lambda} I)^j(\alpha-\beta\i)=0.\]
        特别的，取$j=1$，则$\lambda$是$\sigma_{\mathbf{C}}$的特征值的充分必要条件是$\overline{\lambda}$是$\sigma_{\mathbf{C}}$的特征值.
    \end{enumerate}
\end{theorem}

\begin{proof}
    \begin{enumerate}
        \item 充分性：设$\lambda$是$\sigma$的特征值，$\alpha\in V$是$\sigma$属于$\lambda$的特征向量，则$\sigma_{\mathbf{C}}(\alpha)=\sigma(\alpha)=\lambda\alpha$，因此$\alpha$也是$\sigma_{\mathbf{C}}$属于$\lambda$的特征向量，因此$\lambda$是$\sigma_{\mathbf{C}}$的特征值.

              必要性：设$\lambda$是$\sigma_{\mathbf{C}}$的特征值，$\alpha+\beta\i\in V_{\mathbf{C}}(\alpha,\beta\in V)$是$\sigma_{\mathbf{C}}$属于$\lambda$的特征向量，则
              \[\sigma_{\mathbf{C}}(\alpha+\beta\i)=\sigma(\alpha)+\sigma(\beta)\i=\lambda(\alpha+\beta\i).\]
              展开括号，我们有
              \[\sigma(\alpha)=\lambda\alpha,\enspace \sigma(\beta)=\lambda\beta.\]
              因为$\alpha+\beta\i\neq 0$，所以$\alpha,\beta$不全为零，因此至少有一个不为零的向量是$\sigma$属于$\lambda$的特征向量，因此$\lambda$是$\sigma$的特征值.
        \item 充分性：设$\alpha,\beta\in V$使得$\sigma(\alpha)=a\alpha-b\beta,\enspace \sigma(\beta)=b\alpha+a\beta$，则
              \[\sigma_{\mathbf{C}}(\alpha+\beta\i)=\sigma(\alpha)+\sigma(\beta)\i=(a\alpha-b\beta)+(b\alpha+a\beta)\i=(a+b\i)(\alpha+\beta\i),\]
              因此$\alpha+\beta\i$是$\sigma_{\mathbf{C}}$属于$\lambda$的特征向量，因此$\lambda$是$\sigma_{\mathbf{C}}$的特征值.

              必要性：设$\lambda=a+b\i$是$\sigma_{\mathbf{C}}$的特征值，$\alpha+\beta\i\in V_{\mathbf{C}}(\alpha,\beta\in V)$是$\sigma_{\mathbf{C}}$属于$\lambda$的特征向量，则
              \[\sigma_{\mathbf{C}}(\alpha+\beta\i)=\sigma(\alpha)+\sigma(\beta)\i=(a+b\i)(\alpha+\beta\i).\]
              展开括号，我们有$\sigma(\alpha)=a\alpha-b\beta,\enspace \sigma(\beta)=b\alpha+a\beta$. 因为$\alpha+\beta\i\neq 0$，所以$\alpha,\beta$不全为零，故必要性成立.
        \item 对$j$用归纳法，若$j=0$，由于一个算子的$0$次幂为恒等算子，因此结论即为$u+v\i=0 \iff u-v\i=0$，显然成立.

              假设$j=k-1$时结论成立，考虑$j=k$的情况. 注意到
              \[
              (\sigma_{\mathbf{C}}-\lambda I)^k(\alpha+\beta\i)=(\sigma_{\mathbf{C}}-\lambda I)^{k-1}(\sigma_{\mathbf{C}}-\lambda I)(\alpha+\beta\i),
              \]
              而
              \begin{gather*}
                (\sigma_{\mathbf{C}}-\lambda I)(\alpha+\beta\i)=(\sigma(\alpha)-a\alpha+b\beta)+(\sigma(\beta)-b\alpha-a\beta)\i, \\
                (\sigma_{\mathbf{C}}-\overline{\lambda} I)(\alpha-\beta\i)=(\sigma(\alpha)-a\alpha+b\beta)-(\sigma(\beta)-b\alpha-a\beta)\i.
              \end{gather*}
              故由归纳假设可知
              \[
              (\sigma_{\mathbf{C}}-\lambda I)^{k-1}((\sigma(\alpha)-a\alpha+b\beta)+(\sigma(\beta)-b\alpha-a\beta)\i)=0
              \]
              当且仅当
              \[
              (\sigma_{\mathbf{C}}-\overline{\lambda} I)^{k-1}((\sigma(\alpha)-a\alpha+b\beta)-(\sigma(\beta)-b\alpha-a\beta)\i)=0
              \]
              即\[(\sigma_{\mathbf{C}}-\lambda I)^k(\alpha+\beta\i)=0 \iff (\sigma_{\mathbf{C}}-\overline{\lambda} I)^k(\alpha-\beta\i)=0.\]
              由归纳法知结论成立.
    \end{enumerate}
\end{proof}

\autoref{thm:复化线性变换的特征值与特征向量} 的结论表明，实线性空间上线性变换的复化的非实特征值成对出现，并且它们对应的特征向量也成共轭. 这一结论我们在\nameref{chap:多项式的进一步讨论}一节还会进一步讨论.
...
\begin{summary}
    ...
    本讲我们介绍了相似的定义与性质，并介绍了第一个且是最简单的相似标准形——对角矩阵. 我们介绍了线性变换和矩阵可对角化的定义以及二者的统一性，介绍了如何求解线性变换/矩阵的对角化问题. 我们也探讨了线性变换/矩阵可对角化的几个充分必要条件，并通过大量的例题运用了这些条件. 最后我们介绍了幂等矩阵、幂零矩阵、算子的平方根问题与复化问题等内容，它们有很多值得探讨的性质，适合作为本讲的一个应用.
...
\begin{exercise}
    ...
    \begin{exgroup}
        ...
        \begin{answer}
            ...
            \end{pmatrix}$.
        \end{answer}
        \item 证明\autoref{ex:复化空间的维数}.
        \begin{answer}
            由于$v_1,\ldots,v_n$为实线性空间$V$的一组基，因此在复线性空间$V_{\mathbf{C}}$中，$\spa(v_1,\ldots,v_n)$包含了所有的$v_1,\i v_1,\ldots,v_n,\i v_n$，自然可以张成任意的$u+v\i,\enspace u,v\in V$，因此$v_1,\ldots,v_n$张成了整个$V_{\mathbf{C}}$. 故只需证明它们在$V_\mathbf{C}$中线性无关.

            设$\lambda_1,\ldots,\lambda_n\in\mathbf{C}$使得
            \[\lambda_1 v_1+\cdots+\lambda_n v_n=0,\]
            则\[\operatorname{Re}(\lambda_1)v_1+\cdots+\operatorname{Re}(\lambda_n)v_n=0,\operatorname{Im}(\lambda_1)v_1+\cdots+\operatorname{Im}(\lambda_n)v_n=0.\]
            由于$v_1,\ldots,v_n$在$V$中线性无关，因此$\operatorname{Re}(\lambda_1)=\cdots=\operatorname{Re}(\lambda_n)=0$，$\operatorname{Im}(\lambda_1)=\cdots=\operatorname{Im}(\lambda_n)=0$，从而$\lambda_1=\cdots=\lambda_n=0$. 因此$v_1,\ldots,v_n$在$V_{\mathbf{C}}$中线性无关，从而$v_1,\ldots,v_n$是$V_{\mathbf{C}}$的一组基.
        \end{answer}
        \item 证明\autoref{ex:复化线性变换的矩阵表示}.
        \begin{answer}
            略微有点反直觉，但只需注意到基在复化之后仍为基，且对$\forall u\in V$，$\sigma_{\mathbf{C}}(u)=\sigma(u)$，设$A$是$\sigma$在基$(v_1,\ldots,v_n)$下的矩阵表示，则\[\sigma_{\mathbf{C}}(v_1,\ldots,v_n)=\sigma(v_1,\ldots,v_n)=(v_1,\ldots,v_n)A,\]
            从而$A$也是$\sigma_{\mathbf{C}}$在$v_1,\ldots,v_n$下的矩阵表示.
        \end{answer}
    \end{exgroup}
