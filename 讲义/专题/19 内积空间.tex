...
\begin{proof}
    ...
    若 $v \ne \mathbf{0}$，根据\autoref{thm:内积空间上向量的正交分解}，令 $ c = \dfrac{\langle u, v\rangle}{\lVert v \rVert^2}, \enspace w = u - \dfrac{\langle u, v\rangle}{\lVert v \rVert^2}v$.
    ...
    因为 $\lVert w \rVert^2 \geqslant 0$，所以 $\lVert u \rVert^2 \geqslant \lvert c \rvert^2\lVert v \rVert^2$.
    ...
    当等号成立，则 $\lVert w \rVert^2 = 0$，即 $ w = u - cv = \mathbf{0}$.

    可得 $ u = c v $， 说明 $ u $ 是 $ v $ 的标量倍.
\end{proof}
...
\begin{proof}
考虑向量 $u - c v$ 的范数的平方，其中 $c$ 是一个标量.

为方便起见，若 $\langle u, v \rangle \ne 0$，可设 $\alpha = \frac{\overline{\langle u, v \rangle}}{|\langle u, v \rangle|}$，然后\textbf{考虑二次三项式 $\|u - t \alpha v\|^2$}，其中 $t \in \mathbb{R}$。

展开范数平方，并利用 $\|u - t \alpha v\|^2 \ge 0$ 恒成立，通过\textbf{判别式 $\Delta \le 0$} 即可得证。
\end{proof}

借助 Cauchy-Schwarz 不等式，我们可以得到三角不等式：

\begin{theorem}{}{内积空间上的三角不等式}
    设 $u, v \in V$. 则 $\lVert u, v \rVert \leqslant \lVert u \rVert + \lVert v \rVert$. 等号成立当且仅当 $u, v$ 之一是另一个的非负标量倍.
\end{theorem}

\begin{proof}
    我们考察 $\lVert u + v \rVert^2$:
    \begin{align*}
    \lVert u + v \rVert^2 & = \langle u + v, u + v \rangle \\
                          & = \langle u, u \rangle + \langle u, v \rangle + \langle v, u \rangle + \langle v, v \rangle \\
                          & = \lVert u \rVert^2 + \langle u, v \rangle + \overline{\langle u, v \rangle} + \lVert v \rVert^2 \\
                          & = \lVert u \rVert^2 + 2 \text{Re}(\langle u, v \rangle) + \lVert v \rVert^2 \\
...
\begin{proof}
    计算 $\lVert u + v \rVert^2, \lVert u - v \rVert^2$：
    $$\lVert u + v \rVert^2 = \langle u + v, u + v \rangle = \lVert u \rVert^2 + \langle u, v \rangle + \langle v, u \rangle + \lVert v \rVert^2$$
    $$\lVert u - v \rVert^2 = \langle u - v, u - v \rangle = \lVert u \rVert^2 - \langle u, v \rangle - \langle v, u \rangle + \lVert v \rVert^2$$
    将两式相加得证.
\end{proof}
...
\begin{solution}
    \begin{enumerate}
        ...
        我们展开等式右侧的两项，利用线性算子 $T$ 的线性性：
        \begin{align*}
            \langle T(u + v), u + v\rangle - \langle T(u - v), u - v\rangle
            & = \langle Tu + Tv, u + v \rangle - \langle Tu - Tv, u - v \rangle \\
            & = \left(\langle Tu, u \rangle + \langle Tu, v \rangle + \langle Tv, u \rangle + \langle Tv, v \rangle\right) \\
            & \quad - \left(\langle Tu, u \rangle - \langle Tu, v \rangle - \langle Tv, u \rangle + \langle Tv, v \rangle\right) \\
            & = \langle Tu, v \rangle - (-\langle Tu, v \rangle) + \langle Tv, u \rangle - (-\langle Tv, u \rangle) \\
            & = 2\langle Tu, v \rangle + 2\langle Tv, u \rangle \\
        ...
        首先，我们利用复数内积 $\langle v, u \rangle = \overline{\langle u, v \rangle}$ 得到实部项：
        $$\lVert u + v \rVert^2 - \lVert u - v \rVert^2 = 4 \text{Re}(\langle u, v \rangle)$$
        接下来计算虚部项 $\lVert u + \i v \rVert^2 - \lVert u - \i v \rVert^2$:
        \begin{align*}
            \lVert u + \i v \rVert^2 - \lVert u - \i v \rVert^2
            & = \langle u + \i v, u + \i v \rangle - \langle u - \i v, u - \i v \rangle \\
            & = \left(\lVert u \rVert^2 - \i \langle u, v \rangle + \i \langle v, u \rangle + \lVert v \rVert^2\right) - \left(\lVert u \rVert^2 + \i \langle u, v \rangle - \i \langle v, u \rangle + \lVert v \rVert^2\right) \\
            & = -2\i \langle u, v \rangle + 2\i \langle v, u \rangle \\
            & = 2\i (\overline{\langle u, v \rangle} - \langle u, v \rangle) \\
            & = 2\i (-2\i \text{Im}(\langle u, v \rangle)) \\
            & = 4 \text{Im}(\langle u, v \rangle)
        \end{align*}
        代入原恒等式右侧：
        \begin{align*}
            \text{RHS} &= \frac{1}{4}\left(\left(4 \text{Re}(\langle u, v \rangle)\right) + \i\left(4 \text{Im}(\langle u, v \rangle)\right)\right) \\
            & = \text{Re}(\langle u, v \rangle) + \i \text{Im}(\langle u, v \rangle) \\
            & = \langle u, v \rangle
        \end{align*}
        ...
        \item $ (19.4) $ 复空间算子恒等式
        我们将右侧分解为 $\frac{1}{4} (\text{Part A} + \i \cdot \text{Part B})$。

        $$\text{Part A} = \langle T(u + v), u + v\rangle - \langle T(u - v), u - v\rangle = 2\langle Tu, v \rangle + 2\langle Tv, u \rangle$$

        \begin{align*}
            \text{Part B} & = \langle T(u + \i v), u + \i v\rangle - \langle T(u - \i v), u - \i v\rangle \\
            & = \left(\langle Tu, u \rangle - \i \langle Tu, v \rangle + \i \langle Tv, u \rangle + \langle Tv, v \rangle\right) \\
            &\quad - \left(\langle Tu, u \rangle + \i \langle Tu, v \rangle - \i \langle Tv, u \rangle + \langle Tv, v \rangle\right) \\
            & = -2\i \langle Tu, v \rangle + 2\i \langle Tv, u \rangle \\
            & = 2\i (\langle Tv, u \rangle - \langle Tu, v \rangle)
        \end{align*}

        将 $\text{Part A}$ 和 $\text{Part B}$ 代入右边得到：
        \begin{align*}
            \frac{1}{4}\left(\text{Part A} + \i \cdot \text{Part B}\right)
            & = \frac{1}{4}\left(\left(2\langle Tu, v \rangle + 2\langle Tv, u \rangle\right) + \i \left(2\i (\langle Tv, u \rangle - \langle Tu, v \rangle)\right)\right) \\
            & = \frac{1}{4}\left(2\langle Tu, v \rangle + 2\langle Tv, u \rangle - 2\langle Tv, u \rangle + 2\langle Tu, v \rangle\right) \\
            & = \frac{1}{4}\left(4\langle Tu, v \rangle\right) \\
            & = \langle Tu, v \rangle
        \end{align*}
    \end{enumerate}
\end{solution}
...
\begin{proof}
    根据向量范数平方的定义 $\lVert u \rVert^2 = \langle u, u \rangle$，设 $u = a_1e_1 + \cdots + a_m e_m$. 我们计算 $\lVert u \rVert^2$:
    \begin{align*}
        \lVert u \rVert^2 & = \left\langle \sum_{i=1}^{m} a_i e_i, \sum_{j=1}^{m} a_j e_j \right\rangle \\
        & = \sum_{i=1}^{m} \sum_{j=1}^{m} \left\langle a_i e_i, a_j e_j \right\rangle \\
        & = \sum_{i=1}^{m} \sum_{j=1}^{m} a_i \overline{a_j} \left\langle e_i, e_j \right\rangle
    \end{align*}
    由于 $\left\{e_1, \ldots, e_m\right\}$ 是标准正交向量组，内积 $\left\langle e_i, e_j \right\rangle$ 满足：
    \[
        \left\langle e_i, e_j \right\rangle = \begin{cases} 1 & i = j \\ 0 & i \ne j \end{cases}
    \]
    因此，在所有求和项中，只有当 $i = j$ 时，项的值才非零：
    \begin{align*}
        \lVert u \rVert^2 &= \sum_{i=1}^{m} a_i \overline{a_i} \left\langle e_i, e_i \right\rangle \\
        & = \sum_{i=1}^{m} a_i \overline{a_i} \cdot 1 \\
        & = \sum_{i=1}^{m} \lvert a_i \rvert^2
    \end{align*}
    即
...
\begin{proof}
    ...
    我们计算 $\lVert v \rVert^2$，并将上一步得出的 $v$ 的展开式代入：
    \begin{align*}
        \lVert v \rVert^2 = \langle v, v \rangle & = \left\langle \sum_{i=1}^{n} \langle v, e_i \rangle e_i, \sum_{j=1}^{n} \langle v, e_j \rangle e_j \right\rangle \\
        & = \sum_{i=1}^{n} \sum_{j=1}^{n} \langle v, e_i \rangle \overline{\langle v, e_j \rangle} \langle e_i, e_j \rangle \\
        & = \sum_{i=1}^{n} \langle v, e_i \rangle \overline{\langle v, e_i \rangle} \langle e_i, e_i \rangle \\
        & = \sum_{i=1}^{n} \lvert \langle v, e_i \rangle \rvert^2
    \end{align*}
    即
    \[ \lVert v \rVert^2 = \lvert \langle v, e_1\rangle \rvert^2 + \cdots + \lvert \langle v, e_n\rangle \rvert^2. \]
    两个结论均得证.
\end{proof}
...
\begin{solution}
    ...
    $\lVert v \rVert^2 = \sum_{i=1}^{m} \lvert \langle v, e_i \rangle \rvert^2$.
    构造向量 $u = \sum_{i=1}^{m} \langle v, e_i \rangle e_i$. 显然 $u \in W$.

    令 $w = v - u$. 证明 $w$ 与 $u$ 正交，即 $w = v - u$ 与所有 $e_j$ 正交：
    \[
        \langle w, e_j \rangle = \left\langle v - \sum_{i=1}^{m} \langle v, e_i \rangle e_i, e_j \right\rangle = \langle v, e_j \rangle - \sum_{i=1}^{m} \langle v, e_i \rangle \langle e_i, e_j \rangle
    \]
    由于 $\langle e_i, e_j \rangle = \delta_{ij}$，
    \[
        \langle w, e_j \rangle = \langle v, e_j \rangle - \langle v, e_j \rangle \langle e_j, e_j \rangle = 0
    \]
    因此 $w$ 与 $u$ 正交，$\langle w, u \rangle = 0$.

    计算 $\lVert w \rVert^2$:
    \begin{align*}
        \lVert w \rVert^2 &= \langle w, w \rangle = \langle w, v - u \rangle = \langle w, v \rangle - \langle w, u \rangle \\
        &= \langle w, v \rangle = \langle v - u, v \rangle = \langle v, v \rangle - \langle u, v \rangle \\
        &= \lVert v \rVert^2 - \sum_{i=1}^{m} \langle v, e_i \rangle \langle e_i, v \rangle \\
        &= \lVert v \rVert^2 - \sum_{i=1}^{m} \langle v, e_i \rangle \overline{\langle v, e_i \rangle} \\
        &= \lVert v \rVert^2 - \sum_{i=1}^{m} \lvert \langle v, e_i \rangle \rvert^2
    \end{align*}
    根据假设 $\lVert v \rVert^2 = \sum_{i=1}^{m} \lvert \langle v, e_i \rangle \rvert^2$，故 $\lVert w \rVert^2 = 0$.
    由范数正定性 $w = \vec{0}$，即 $v = u$.

    由于 $u \in W$，所以 $v \in \spa(e_1, \ldots , e_m)$.

    \\

    充分性 ($\Leftarrow$):

    $v \in \spa(e_1, \ldots , e_m)$.
    由于 $\{e_1, \ldots, e_m\}$ 是标准正交组，且 $v \in \spa(e_1, \ldots , e_m)$，故 $v$ 可以唯一地表示为：
    \[ v = \sum_{i=1}^{m} a_i e_i \]
    其中 $a_i = \langle v, e_i \rangle$.
    根据\autoref{thm:勾股定理推广} $\lVert a_1e_1 + \cdots + a_m e_m\rVert^2 = \lvert a_1 \rvert^2 + \cdots + \lvert a_m \rvert^2$:
    \[ \lVert v \rVert^2 = \left\lVert \sum_{i=1}^{m} \langle v, e_i \rangle e_i \right\rVert^2 = \sum_{i=1}^{m} \lvert \langle v, e_i \rangle \rvert^2 \]
    充分性得证.
\end{solution}
...
\begin{example}{}{}
    求 $\mathbf{R}[x]_3$ 的一组标准正交基，内积定义为 $\langle p, q \rangle = \displaystyle\int_{-1}^1 p(x)q(x)\,\mathrm{d}x$.
\end{example}
...
\begin{solution}
    ...
    首先计算 $\lVert v_1 \rVert^2$:
    \[ \lVert v_1 \rVert^2 = \langle 1, 1 \rangle = \int_{-1}^1 1 \,\mathrm{d}x = 2 \]
    \[ e_1 = \frac{v_1}{\lVert v_1 \rVert} = \frac{1}{\sqrt{2}} \]
    ...
    单位化：
    \[ \lVert u_2 \rVert^2 = \langle x, x \rangle = \int_{-1}^1 x^2 \,\mathrm{d}x = \left[\frac{1}{3}x^3\right]_{-1}^1 = \frac{2}{3} \]
    \[ e_2 = \frac{u_2}{\lVert u_2 \rVert} = \frac{x}{\sqrt{2/3}} = \sqrt{\frac{3}{2}} x \]
