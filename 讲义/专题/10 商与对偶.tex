...
\begin{proof}
    又一次我们遇到了映射套映射的情况，那当然类似于之前对偶映射的讨论，我们的处理手段就是对需要定义的映射进行``取值计算''. 于是我们取$\psi$在$v\in V$上的值，得到$\psi(v)$，它是一个将线性泛函映到实数的线性泛函. 这仍然是映射套映射的情况，于是我们不妨更进一步，取在$f:V^*\to\R$上的值，我们有$(\psi(v))(f)$是一个实数，这时我们要素齐全，直接令
