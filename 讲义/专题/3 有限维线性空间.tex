...
\begin{proof}
    设 $\beta_j = \sum_{i = 1}^r \lambda_{ij} \alpha_i, \lambda_{ij} \in \mathbf{F}, j = 1, 2, \ldots, s$. 由线性相关的定义，再设
    ...
    \[\sum_{j = 1}^s \lambda_{ij}x_j = 0, \ i = 1 , \ldots, r.\]
...
\begin{proof}
    对于线性空间$\mathbf{C(C)}$中的任一向量 $a + bi$，其总可以被向量 $\alpha = 1$表示，数乘系数为 $\lambda = a + bi$，所以$\mathbf{C(C)}$维数为1；对于线性空间$\mathbf{C(R)}$，向量组 $\alpha = 1, \beta = i$线性无关，且任一向量 $u = a + bi = a·\alpha + b·\beta$，可被 $\alpha, \beta$表示，所以$\mathbf{C(R)}$维数为2.
\end{proof}
