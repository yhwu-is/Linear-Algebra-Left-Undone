...
\begin{example}{}{子空间运算}
    ...
    进一步考察并集，事实上显然$W_1 \cup W_2$得到的集合表示$xOz$和$yOz$平面上所有的点. 不难发现，$W_1 \cup W_2$得到的集合关于向量加法、数乘运算并不封闭，例如只需取$\alpha_2+\alpha_3=(1,1,0)$就不在$W_1 \cup W_2$中，因此不再是$\mathbf{R}^3$的子空间.
